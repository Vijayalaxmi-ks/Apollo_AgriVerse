...
# Portable paths: env var, then project data/, then original Windows path
_CANDIDATES = [
    Path(__file__).resolve().parents[2] / "data" / "weather_raw_dataset.csv",
    Path(__file__).resolve().parents[3] / "data" / "weather_raw_dataset.csv",
    Path(r"D:/Apollo_AgriVerse/02_Datasets/Raw/weather/weather_raw_dataset.csv"),
]
WEATHER_FILE = next((str(c) for c in _CANDIDATES if c.exists()), str(_CANDIDATES[0]))
try:
